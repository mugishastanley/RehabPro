...
{
  double pose[7]={-0.0387236, 0.770225, -0.0529509, -0.275336, -0.847398, -0.140291, 0.431771};
  double* T = new double[16];
  double* Tp = new double[16];

  //finding the trans mat from the given pose
  convert(pose, T);
  Tp =  T;
  double qp_sols[8*6];
  int num_sols;

  // finding the inverse for the given pose
  num_sols = inverse(Tp, qp_sols);

  // printing the joint values
  double qp1[6];
  for (int i = 0; i < 6; i++)
  {
      qp1[i] = qp_sols[i];
  ...
  // Transmat for the obtained joints q
  forward(qp1, T);
 ...
 for(int i=0;i<1;i++)
    printf("%1.6f %1.6f %1.6f %1.6f %1.6f %1.6f\n",
       qp_sols[i*6+0], qp_sols[i*6+1], qp_sols[i*6+2], qp_sols[i*6+3], qp_sols[i*6+4], qp_sols[i*6+5]);

 for(int i=0;i<1;i++)
    printf("%1.6f %1.6f %1.6f %1.6f %1.6f %1.6f\n",
       qp1[0], qp1[1], qp1[2], qp1[3], qp1[4], qp1[5]);
